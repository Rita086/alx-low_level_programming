 ...
 * @e_ident: A pointer to an array containing the ELF magic numbers.
 *
 * Description: Magic numbers are detached by spaces
 */
void print_magic(unsigned char *e_ident)
{
	int index;
 ...
 *
 * Description: If the file cannot be terminated - exit code 98.
 */
void close_elf(int elf)
{
	if (close(elf) == -1)
 ...
 * Return: 0 on success.
 *
 * Description: If the is not an ELF File or function fails - exit code 98.
 */
